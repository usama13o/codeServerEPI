...
from .utils import load_nifti_img, check_exceptions, is_image_file, open_image_np, open_image_np_bw,open_target_np, open_target_np_glas, open_target_np_peso;                   
import random

class siim_acr_dataset(data.Dataset):
    ...
    def __init__(self, root_dir, split, transform=None, preload_data=False,train_pct=0.8,balance=True):
        super( siim_acr_dataset, self).__init__()
        ...
        if self.preload_data:
            ...
            self.raw_images = [open_image_np_bw(ii)[0] for ii in self.image_filenames]
    ...
    def __getitem__(self, index):
        ...
        if not self.preload_data:
            input  = open_image_np_bw(self.image_filenames[index])
