...
# %% creating test data
    ...
    MessageTools.show_yellow(f"saving to ---> output_slides/{extra_path}{slide_num}_stitched_{TARG_PRED}.png")
    if not os.path.exists('output_slides/'):
      os.makedirs('output_slides/')
    np_to_pil(re_tile.astype('bool')).save(f"output_slides/{extra_path}{slide_num}_stitched_{TARG_PRED}.png")
