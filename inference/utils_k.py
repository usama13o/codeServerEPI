...
if mode == "epi":
    ...
    original_file_names = sorted(glob("../input/epithelial-cells-ihc/*"))
    FILTER_DIR_names = sorted(glob("../input/epithelial-cells-ihc/*"))
else:
    original_file_names = sorted(glob("./filtered/*"))
    FILTER_DIR_names = sorted(glob("../input/normal-tifff/"+"/*"))
annotated_test_dir = sorted( glob("./filtered/*"))
annotated_tif_test_dir = sorted(glob("D:\\Other\\DOWNLOADS\\WSIData\\Normal\\GT\\fixed\\*"))
PNG_original_file_names = sorted(glob(PNG_org_DIR+"\*"))
PNG_annotated_file_names =sorted(glob("../input/epithelial-cells-ihc"+"\*"))
...
def score_tiles(path, np_img=None, dimensions=None, small_tile_in_tile=False,SCALE_FACTOR=SCALE_FACTOR,test=False):
 ...
 # this gets the mask of the HE tifs from their counter-parts PNGS (ie this the np of a png iamge) not tif 
    ...
    low = 0
    none = 0
    tile_indices = get_tile_indices(w, h, row_tile_size, col_tile_size)
