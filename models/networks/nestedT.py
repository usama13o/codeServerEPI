...
from models.networks.vit_seg_modeling import DecoderCup, SegmentationHead
...
class Nest(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
        ...
        config= CONFIGS_ViT_seg["testing_nest"]
    ...
    def forward_features(self, x):
        ...
        return x,tuple(outs)
    def forward(self,x):
        return self.forward_decoderCuP(x)
    ...
    def forward_decoderCuP(self, x):
        """ x shape (B, C, H, W)
        """
        h , w = x.size(2),x.size(3)
        x = self.forward_features(x)
        x = self.decodercup.forward(x[0],x[1][:-1])
