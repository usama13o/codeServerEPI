...
#Consts file names
DEST_MAIN_DIR="D:\\Other\\DOWNLOADS\\WSIData\\Normal"
...
annotated_file_names = sorted(glob("F:\Data\peso_dataset\\annotated_test_dir\\*"))
original_file_names = sorted(glob("F:\Data\peso_dataset\\slides_tif\\*"))
annotated_test_dir = sorted( glob("F:\Data\peso_dataset\\scaled_annotated_test_dir\\*"))
annotated_tif_test_dir = sorted( glob("F:\Data\peso_dataset\\annotated_test_dir\\*"))
PNG_original_file_names = sorted(glob('D:\Other\DOWNLOADS\WSIData\\PNG'+"\*"))
PNG_annotated_file_names =sorted(glob("D:\Other\DOWNLOADS\WSIData\\PNG_ann"+"\*"))
FILTER_DIR ="F:\\Data\\peso_dataset\\slides_tif\\"
...
SUMMARY_TITLE_FONT_PATH = "Z:\\ae_Salem.ttf"
FONT_PATH =  SUMMARY_TITLE_FONT_PATH
...
def training_slide_to_image(path,DEST_TRAIN_DI=0,scale_factor=1,test=False):
    """
...
# for original [35:-4] , annotated [37:-4]
    ...
    if not os.path.exists(DEST_TRAIN_DI):
        os.makedirs(DEST_TRAIN_DI)
    img.save(img_path)
...
def mask_percent(np_img):
    ...
    Args:
    np_img: Image of mask as a NumPy array.
    Returns:
    The percentage of the NumPy array that is masked.
    """
    if 2 in np.unique(np_img):
      np_img[np_img!=2]=0
    if (len(np_img.shape) == 3) and (np_img.shape[2] == 3):
        np_sum = np_img[:, :, 0] + np_img[:, :, 1] + np_img[:, :, 2]
        mask_percentage = 100 - np.count_nonzero(np_sum) / np_sum.size * 100
        # #print(mask_percentage,np.count_nonzero(np_sum),np_img.size,np.count_nonzero(np_img))
    else:
...
def get_annotation_path(org_path):
    ...
    slide_num=org_path.split("_")[2]
    match = [s for s in PNG_annotated_file_names if slide_num in s][0]
...
def save_display_tile(tile, save=True, display=False,isThereTest=False):
    ...
    tile_train_mask,np_ann = tile.get_np_ann_tile()
    ...
    if isThereTest: 
      ...
      # #print(np.unique(np.array(tile_train_mask)))
    if save:
...
def get_mask_for_test(path):
  ...
  # #print(match)
  match= open_image_np(match)
  # #print(np.unique(match))
  return match 
...
def score_tiles(path, np_img=None, dimensions=None, small_tile_in_tile=False,SCALE_FACTOR=SCALE_FACTOR,test=False):
    ...
    Args:
    path: the path to scaled wsi image
 ...
 # this gets the mask of the HE tifs from their counter-parts PNGS (ie this the np of a png iamge) not tif 
    ...
    low = 0
    none = 0
    #TODO for some reason the order for HE peso slides works with (h,w) but the NOtt data works with (w,h)
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
    ...
    #TODO changed the order of positional params ,num_row_tiles ,num_col_tiles,--->num_col_tiles,,num_row_tiles 
