...
if not os.path.exists(f'/content/artifacts/{MODEL_BASE}:v{MODEL_VER}/export.pkl'):
  MessageTools.show_yellow("Gettiing model .. Downloading ... ")
  run = wandb.init()
  artifact = run.use_artifact(f'usama_ml/EPISEG/{MODEL_BASE}:v{MODEL_VER}', type='model')
...
try:
  ...
  dls = manual.dataloaders(f'/content/{DATALOADER_PATH}',bs=2)

  new_learner = load_learner(f"artifacts/{MODEL_BASE}:v{MODEL_VER}/export.pkl")
  new_learner.dls=dls
