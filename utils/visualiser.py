...
class Visualiser():
    def __init__(self, opt, save_dir, filename='loss_log.txt',resume=False,config=None):
        ...
        if self.use_wandb:
            ...
            self.run=wandb.init(project='EPISEG',name=f'Guacamole_{opt.run_name}_{now.strftime("%m/%d/%Y, %H:%M")}',resume=resume,config=config,tags=[config['tags']])
